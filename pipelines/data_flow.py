...
def task_cargar_espacial(cfg: ExperimentConfig):
    ...
    logger.info(f"Capas espaciales: {len(gdf_cuencas)} cuencas | {len(gdf_mpios)} municipios")
...
def task_construir_grid(gdf_asignado, gdf_cuencas, df_chirps_semanal, cfg: ExperimentConfig):
    logger = get_run_logger()
    ...
    df_grid_pa = generate_pseudo_absences(
        df_grid_full, df_precip_pa, gdf_cuencas,
...
def task_guardar_datasets(
    ...
    if cols_nulas:
        raise ValueError(f"Features CHIRPS completamente nulas tras merge: {cols_nulas}")
    ...
    logger.info(f"Grid PA   guardado: {path_pa.name}   shape={df_pa_enriquecido.shape} pos={n_pos_pa}")
    logger.info(f"Grid full guardado: {path_full.name} shape={df_full_enriquecido.shape} pos={n_pos_full}")
...
def full_pipeline(
    ...
    """
    from training_flow import training_pipeline  # import local para evitar ciclo al nivel de módulo
