...
def main() -> None:
    ...
    print(
        f"Eventos deslizamiento Antioquia {cfg.periodo.anio_inicio}-{cfg.periodo.anio_fin}: {len(df_ant)}"
    )
    ...
    print(
        f"Cuencas con al menos 1 evento: {gdf_asignado['HYBAS_ID'].nunique()} / {len(gdf_cuencas)}"
    )
    ...
    # ERA5 no disponible localmente → columna NaN (SimpleImputer la maneja en el pipeline)
    if "soil_moisture_14d" not in df_model.columns:
        df_model["soil_moisture_14d"] = np.nan
    ...
    print(
        f"  Shape: {df_model.shape} | Positivos: {n_pos:,} ({100 * n_pos / len(df_model):.2f}%)"
    )
    ...
    if "soil_moisture_14d" not in df_full.columns:
        df_full["soil_moisture_14d"] = np.nan
    ...
    print(
        f"  Shape: {df_full.shape} | Positivos: {n_pos_full:,} ({100 * n_pos_full / len(df_full):.2f}%)"
    )
    print(f"\n{'=' * 60}")
