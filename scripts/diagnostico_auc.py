...
feature_val = df_model.loc[
    df_model["anio_semana"] == semana_ejemplo, "precip_acum_14d"
].iloc[0]
...
if len(dias_semana) > 0:
    print(
        f"  Rango de fechas: {dias_semana['fecha'].min().date()} -> {dias_semana['fecha'].max().date()}"
    )
    print(f"  precip_acum_14d en el modelo: {feature_val:.1f} mm")
    print(
        f"  Suma precip_mm solo de esa semana: {dias_semana['precip_mm'].sum():.1f} mm"
    )
    ...
    print(
        f"    Incluye dias de semana W: "
        f"{(ventana_14d['anio_semana'] == semana_ejemplo).sum()} de 7"
    )
    print(
        f"    Incluye dias de semana W-1: "
...
precip_p25 = df_model.loc[df_model["deslizamiento"] == 0, "precip_acum_14d"].quantile(
    0.75
)
...
if min_pos > max_neg:
    print(
        "  [FAIL] NO HAY SOLAPAMIENTO: positivos y negativos son perfectamente separables"
    )
...
for i in range(len(bins) - 1):
    ...
    print(
        f"    proba [{bins[i]:.1f}?{bins[i + 1]:.1f}): {mask.sum():4d} instancias "
...
print(
    f"      n_train={mask_train.sum():,} (pos={y_tr_ho.sum():,}) | "
...
try:
    ...
    mask_ungrd = (df_ungrd["departamento"].str.upper() == "ANTIOQUIA") & (
    ...
    static_cols = [
        c
        for c in ["HYBAS_ID", "SUB_AREA", "UP_AREA", "DIST_MAIN", "ORDER"]
    ...
    mask_test_full = (
        df_full["anio_semana"].apply(lambda p: p.start_time.year) >= SPLIT_YEAR
    )
    ...
    print(
        f"      n_test={len(X_te_full):,} (pos={y_te_full.sum():,}, "
        f"neg={(y_te_full == 0).sum():,})"
    )
    print(f"      AUC={auc_full:.4f}  Recall={rec_full:.4f}  Precision={prec_full:.4f}")
    ...
    print(
        f"    Degradacion             : -{delta:.4f} ({delta / auc_pa * 100:.1f}% drop)"
    )
