...
def _cx(draw: ImageDraw.Draw, text: str, y: int, font,
        color: tuple = TEXT) -> int:
...
def _glow(img: Image.Image, cx: int, cy: int, color: tuple,
          steps=((500, 12), (360, 24), (220, 44), (120, 72))) -> Image.Image:
...
def _phone_frame(draw: ImageDraw.Draw, x: int, y: int,
                 pw: int, ph: int, screen_color: tuple = (10, 18, 32)) -> tuple:
    ...
    # Notch
    nx = sx + (sw - notch_w) // 2
    draw.rounded_rectangle([nx, sy, nx+notch_w, sy+36], radius=18, fill=(22, 32, 54))
...
def _cuencas_map(draw: ImageDraw.Draw, x0: int, y0: int,
                 cols: int, rows: int, cell: int, seed: int = 42) -> None:
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            if col == 0 and row < 5: continue
            if col <= 1 and row > cols - 3: continue
            if col >= cols - 2 and row < 4: continue
            r = random.random()
            cum = 0.0
...
def _logo(draw: ImageDraw.Draw, cx: int, cy: int, size: int = 120) -> None:
    ...
    draw.ellipse([cx-int(ew*0.45), cy-int(eh*0.45),
                  cx+int(ew*0.45), cy+int(eh*0.45)], fill=(8, 40, 38))
    draw.ellipse([cx-int(ew*0.22), cy-int(eh*0.22),
                  cx+int(ew*0.22), cy+int(eh*0.22)], fill=TEXT)
...
def _slide_logo() -> Image.Image:
    ...
    img = _glow(img, W//2, H//2 - 100,
                ACCENT, ((480, 10), (340, 20), (210, 38), (110, 62)))
...
def _slide_app_mapa() -> Image.Image:
    ...
    sx, sy, sw, sh = _phone_frame(draw, ph_x, ph_y, ph_w, ph_h,
                                  screen_color=(8, 16, 28))
    ...
    screen_draw.text((sx + 80, app_bar_y + 18),
                     "VigilantIA", fill=TEXT, font=_font(28, bold=True))

    # Notificacion activa en app bar
    notif_x = sx + sw - 170
    screen_draw.rounded_rectangle(
        [notif_x, app_bar_y + 16, notif_x + 148, app_bar_y + 54],
        radius=20, fill=RED)
    screen_draw.text((notif_x + 18, app_bar_y + 22),
                     "2 ALERTAS", fill=TEXT, font=_font(20, bold=True))
    ...
    screen_draw.rounded_rectangle(
        [sx + 12, alert_y, sx + sw - 12, alert_y + 100],
        radius=14, fill=(30, 10, 10))
    screen_draw.rounded_rectangle(
        [sx + 12, alert_y, sx + sw - 12, alert_y + 6],
        radius=3, fill=RED)
    screen_draw.text((sx + 28, alert_y + 12),
                     "RIESGO ALTO", fill=RED, font=_font(24, bold=True))
    screen_draw.text((sx + 28, alert_y + 50),
                     "Cuenca 4100561480 · Norte", fill=MUTED, font=_font(20))
    screen_draw.text((sx + sw - 110, alert_y + 30),
                     "Ver >", fill=ACCENT, font=_font(22, bold=True))

    # Nav bar
    nav_y = sy + sh - 62
    screen_draw.rounded_rectangle(
        [sx, nav_y, sx+sw, sy+sh], radius=0, fill=(10, 22, 40))
    nav_items = [("Mapa", True), ("Alertas", False), ("Zonas", False), ("Config", False)]
    nw = sw // len(nav_items)
    for i, (label, active) in enumerate(nav_items):
        nx = sx + i * nw + nw // 2
        color = ACCENT if active else MUTED
        screen_draw.text((nx - _tw(screen_draw, label, _font(18)) // 2, nav_y + 16),
                         label, fill=color, font=_font(18))
...
def _slide_notificacion() -> Image.Image:
    ...
    draw.rounded_rectangle([notif_x-4, y-4, notif_x+notif_w+4, y+340],
                            radius=30, fill=(20, 30, 50))

    # Notificacion card
    draw.rounded_rectangle([notif_x, y, notif_x+notif_w, y+320],
                            radius=26, fill=(16, 26, 46))
    draw.rounded_rectangle([notif_x, y, notif_x+notif_w, y+8],
                            radius=4, fill=RED)

    # App icon en la notif
    _logo(draw, notif_x + 52, y + 66, size=36)
    draw.text((notif_x + 100, y + 22),
              APP_NAME, fill=TEXT, font=_font(26, bold=True))
    draw.text((notif_x + 100, y + 58),
              "hace unos segundos", fill=MUTED, font=_font(20))

    draw.text((notif_x + 36, y + 108),
              "Alerta de Riesgo Alto", fill=RED, font=_font(34, bold=True))
    draw.text((notif_x + 36, y + 162),
              "Cuenca Norte Antioquia  (4100561480)", fill=TEXT, font=_font(26))
    draw.text((notif_x + 36, y + 206),
              "Probabilidad 82%  ·  Horizonte: 7 dias", fill=MUTED, font=_font(24))

    # Botones de accion
    btn_y = y + 256
    btn_w = (notif_w - 48) // 2
    draw.rounded_rectangle([notif_x + 16, btn_y, notif_x + 16 + btn_w, btn_y + 48],
                            radius=10, fill=(10, 30, 30))
    _cx_in = notif_x + 16 + btn_w // 2
    draw.text((_cx_in - _tw(draw, "Ver mapa", _font(22, bold=True)) // 2, btn_y + 12),
              "Ver mapa", fill=ACCENT, font=_font(22, bold=True))

    draw.rounded_rectangle([notif_x + 32 + btn_w, btn_y,
                             notif_x + 32 + btn_w*2, btn_y + 48],
                            radius=10, fill=RED)
    cx2 = notif_x + 32 + btn_w + btn_w // 2
    draw.text((cx2 - _tw(draw, "Activar alerta", _font(22, bold=True)) // 2, btn_y + 12),
              "Activar alerta", fill=TEXT, font=_font(22, bold=True))
...
def _slide_usuarios() -> Image.Image:
    ...
    for color, title, sub1, sub2 in users:
        ...
        # Circulo de color
        draw.ellipse([mx + 28, y + uh//2 - 34, mx + 96, y + uh//2 + 34],
                     fill=(*color, 30))
        draw.ellipse([mx + 46, y + uh//2 - 18, mx + 80, y + uh//2 + 18],
                     fill=color)
...
def _slide_cta() -> Image.Image:
    ...
    img = _glow(img, W//2, H//2 - 80, ACCENT,
    ...
    for label, detail in opciones:
        ...
        draw.rounded_rectangle([mx, y, mx+cw, y+oh], radius=16, outline=ACCENT, width=1)
...
def _clip(img: Image.Image, duration: float,
          fi: float = 0.35, fo: float = 0.35) -> ImageClip:
