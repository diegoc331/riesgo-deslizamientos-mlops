...
fig.suptitle(
    "Cobertura temporal de fuentes de datos meteorologicos\nAntioquia 2019-2022",
    fontsize=13,
    fontweight="bold",
)
...
for year in [2019, 2020, 2021, 2022]:
    for m0, m1 in [(3, 5), (9, 11)]:
        ax.axvspan(
            pd.Timestamp(year, m0, 1),
            pd.Timestamp(year, m1, 30),
            alpha=0.07,
            color="#4CAF50",
            zorder=0,
        )
...
ax.fill_between(
    x_fechas,
    chirps_semanal["cobertura_pct"],
    color="#1565C0",
    alpha=0.6,
    label="CHIRPS v2.0 (~100% cobertura diaria)",
)
np.random.seed(42)
...
ax.set_title(
    "Cobertura espacial: CHIRPS (satelital) vs IDEAM (estaciones in-situ)\n"
    "La diferencia justifica el cambio de IDEAM a CHIRPS en la v3 del pipeline",
    fontsize=10,
    loc="left",
)
ax.set_ylim(0, 110)
...
ax.text(
    0.99,
    0.95,
    "CHIRPS cubre ~97 puntos grid/semana\nIDEAM: ~3.4% del area real",
...
ax = axes[2]
ax.bar(
    pd.to_datetime(ev_semanal["fecha_fin"]),
    ev_semanal["n_eventos"],
    color="#D32F2F",
    alpha=0.75,
    width=6,
    label="Eventos deslizamiento UNGRD por semana",
)
ax.set_ylabel("N eventos/semana", fontsize=9)
ax.set_title(
    "Eventos UNGRD - Correlacion temporal con precipitacion", fontsize=10, loc="left"
)
...
componentes = [
    ...
    (
        ...
        -COSTO_SISTEMA,
        "#9575CD",
        COSTO_BASE - danio_tp - danio_fn - danio_fa,
        "sub",
    ),
...
for i, (lbl, val, color, bot, tipo) in enumerate(componentes):
    if tipo == "sub":
        ...
        ax.bar(
            i, val, bottom=0, color=color, edgecolor="white", linewidth=0.8, width=0.65
        )
        ax.text(
            i,
            val / 2,
...
ax.annotate(
    ...
    xy=(6, beneficio * 0.5),
    xytext=(4.3, COSTO_BASE * 0.78),
    arrowprops=dict(arrowstyle="->", color="#2E7D32"),
    fontsize=9,
    color="#2E7D32",
    bbox=dict(
        boxstyle="round,pad=0.3", facecolor="#E8F5E9", alpha=0.9, edgecolor="#2E7D32"
    ),
)
...
leg = [
    ...
    Patch(
        facecolor="#66BB6A",
        label=f"Dano resid. TP ({tp} eventos alertados, red. {int(FACTOR_RED * 100)}%): ${danio_tp:,.0f}M",
    ),
    Patch(
        facecolor="#EF9A9A",
        label=f"Dano total FN ({fn} eventos NO alertados): ${danio_fn:,.0f}M",
    ),
    Patch(
        facecolor="#FFB74D",
        label=f"Falsas alarmas ({fp} FP x $5M COP): ${danio_fa:,.0f}M",
    ),
    Patch(
        facecolor="#9575CD", label=f"Operacion del sistema (4 anos): ${COSTO_SISTEMA}M"
    ),
